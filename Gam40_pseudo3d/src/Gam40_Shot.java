...
class Gam40_Shot extends A_GameObject
{
  private static final B_Shape SHAPE = new B_Shape(4,Color.yellow);
  
  private double lifeTime = 1.2;

  public Gam40_Shot(double x, double y, double xDest, double yDest)
  {
    super(x,y,Math.atan2(yDest-y, xDest-x),500,SHAPE);
    this.isMoving = true;
  }
  
  public Gam40_Shot(double x, double y, double a, double s, double time)
  { super(x,y,a,s,SHAPE);
    lifeTime = time;
    this.isMoving = true;
  }
  ...
  { 
	...
	for(int i=0; i<collisions.size(); i++)
	{
	  ...
	  { 
	    Gam40_ZombieKI zombie = (Gam40_ZombieKI)obj;
	    zombie.hasBeenShot();
        this.isLiving=false;
	  }
	}  
