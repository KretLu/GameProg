...
class Gam41_World extends A_World 
{
  ...
  private Gam41_CounterGrenades counterG;
  private Gam41_Counter         counterZ;
  private Gam41_HelpText        helpText;
  ...
  {
    // add the Avatar
	avatar = new Gam41_Avatar(2500,2000);
	...
	for(int x=0; x<5000; x+=1000)
	{
	  ...
	  {
        gameObjects.add(new Gam41_Tree(x+300,y+200,80));
        gameObjects.add(new Gam41_Tree(x+600,y+370,50));
        gameObjects.add(new Gam41_Tree(x+200,y+600,50));
        gameObjects.add(new Gam41_Tree(x+500,y+800,40));
        gameObjects.add(new Gam41_Tree(x+900,y+500,100));
        gameObjects.add(new Gam41_Tree(x+760,y+160,40));
	  }
	}
    ...
    // add one zombie
    gameObjects.add(new Gam41_ZombieKI(100,100));
    
    
    counterZ = new Gam41_Counter(20,40);
    counterG = new Gam41_CounterGrenades(770,40);
    helpText = new Gam41_HelpText(100,400);
  ...
  { 
	...
	if(userInput.isMousePressed && button==3)
	{
      ...
      if(timeSinceLastShot > 0.2)
      {
        ...
        Gam41_Shot shot = new Gam41_Shot(
            avatar.x,
            avatar.y,
  ...
  private void throwGrenade(double x, double y)
  {
    ...
    for(int i=0; i<300; i++)
    {
      ...
      double time  = 0.4+Math.random()*0.6;
      Gam41_Shot shot = new Gam41_Shot(x,y,alfa,speed,time);
      this.gameObjects.add(shot);
    }	  
  ...
  private void createGrenade(double diffSeconds)
  {
	...
	if(spawnGrenade>INTERVAL)
	{
	  ...
	  Gam41_Grenade grenade = new Gam41_Grenade(x,y);
  ...
  private void createZombie(double diffSeconds)
  {
	...
	if(timePassed>INTERVAL)
	{
	  ...
	  // if collisions occur, cancel
	  Gam41_ZombieKI zombie = new Gam41_ZombieKI(x,y);
	  ...
	  Gam41_Counter counter = (Gam41_Counter)textObjects.get(0);
      counter.increment();      
    }
